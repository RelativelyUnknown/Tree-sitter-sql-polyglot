...
def render_markdown(reg: dict, results: dict) -> str:
    ...
    lines = [
        ...
        "",
        "Legend: ✅ implemented · ❌ absent (tracked gap) · — not applicable · · not observable",
        "",
        "## Scores",
        "",
        "| Dialect | Parent | Score | Implemented / Applicable | Agree w/ refs |",
        "|---|---|---|---|---|",
    ]
    for d in ordered:
        ...
        lines.append(f"| {label} | {m.get('parent') or '—'} | **{r['score']}%** "
    ...
    lines += ["", "## Suspect probes",
              "", "_Our grammar accepts these, but **no** independent parser does — "
              "the grammar may be too loose, or the probe is not real SQL._", ""]
    if suspects:
        for s in suspects:
    ...
    lines += ["", "## Confirmed gaps",
              "", "_Our grammar rejects these, but ≥1 independent parser accepts them — "
              "genuinely missing features._", ""]
    if gaps:
        for g in gaps:
    ...
    icon = {"implemented": "✅", "absent": "❌", "not-applicable": "—", "not-observable": "·"}
    for feat in features:
        fid = feat["id"]
        iso = feat.get("iso", "—")
...
def render_step_summary(results: dict) -> str:
    lines = ["## SQL feature coverage (corroborated)", "",
             "| Dialect | Score | Impl / Appl | Agree w/ refs |", "|---|---|---|---|"]
