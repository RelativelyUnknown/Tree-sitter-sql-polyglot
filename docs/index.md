...
hero:
  ...
  tagline: A clean ANSI SQL base plus 22 independently compiled dialect grammars, layered via tree-sitter's grammar(parent, overrides) composition.
  actions:
    - theme: brand
...
features:
  - title: 22 dialects, one grammar family
    details: postgres, mysql, mariadb, oracle, db2, tsql, bigquery, snowflake, redshift, sqlite, duckdb, trino, athena, clickhouse, flink, hive, spark, databricks, cockroachdb, spanner, teradata, and hana — each extending the ANSI base or a real parent dialect.
...
- **[Dialect coverage](/coverage)** — generated on every CI run: per-dialect scores, the inheritance
