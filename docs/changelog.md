...
- **Project renamed and moved.** The fork was cut loose from `tree-sitter-sql-extended` into its own
  repo, `RelativelyUnknown/tree-sitter-sql-polyglot` (GitHub org renamed from `RedPandaMC`). The npm
  scope, the crates.io/PyPI package name and the Go module path all changed to match; anything
  importing under the old names or pointing at the old org needs to update.
...
- `main` now requires a pull request, one code-owner approval, and a passing `ci-passed` check;
  force-pushes and branch deletion are blocked (repo admins may still bypass in an emergency).
- Added a `main` + `release/x.y` branch model, so releases and hotfixes route through a release
  branch rather than landing directly on `main`; `CODEOWNERS`, a PR template, and a `release/**`
  trigger on `ci.yml` back it.
- **npm, crates.io and PyPI publishing switched to OIDC trusted publishing**, dropping the
  long-lived `NPM_TOKEN`/`CARGO_REGISTRY_TOKEN`/`PYPI_API_TOKEN` secrets entirely; each publish job
  now runs under its own GitHub Environment for secret scoping. PyPI's publish step vendors its
  sdist/wheel build directly into `publish.yml` rather than calling a reusable workflow, since OIDC
  can't verify a trusted publisher through one.
- Per-language binding tests were restructured: Go keeps the one full 23-dialect suite (Ubuntu only),
  while Node, Go, Python, Rust and C/CMake each get a lightweight base+postgres check across
  macOS/Ubuntu/Windows, catching an OS- or compiler-specific native-build break without paying to
  build all 22 dialects on every OS. Windows runners were also unified on `windows-2025`
  (`test_go` had drifted onto `windows-latest`).

### Bindings

- **Rust, Node and Python bindings are now per-dialect instead of one bundle carrying all 22.** Rust
  gets one Cargo feature per dialect (default `[]`, `full` for everything); `build.rs` only compiles
  a dialect's `parser.c` when its feature is on. Node and Python each register one native module per
  dialect and only load the one actually imported, instead of dlopening/importing all 22 up front.
  Each dialect's generated `parser.c`/`node-types.json` is brotli-compressed into a committed `.br`
  blob and inflated at build time — the raw multi-dialect crate blew past crates.io's 10MB limit
  (17.8MB gzip-compressed); brotli gets the published crate down to 7.9MB.
- **Go and Swift bindings** get the same per-dialect, lazily-loaded treatment, using each ecosystem's
  own native mechanism instead of a feature-flag system: one importable Go subpackage per dialect
  (`bindings/go/postgres`, ...) with its own `#cgo` directive, so importing one never compiles the
  other 21's C sources; one SwiftPM target and library product per dialect in `Package.swift`
  (`TreeSitterSqlPostgres`, ...). Both bindings previously exposed only the base grammar and had no
  CI coverage at all; Go now gets a full 23-dialect build+test matrix and Swift builds and tests on
  macOS in CI.
- **C/CMake** now supports every dialect, not just base: one CMake option per dialect
  (`-DTREE_SITTER_SQL_<DIALECT>=ON`, or `-DTREE_SITTER_SQL_FULL=ON` for all 22 at once), mirroring
  Cargo's feature-flag shape. `CMakeLists.txt` and `bindings/c/` are now generated by
  `scripts/generate-bindings.js` alongside the other four bindings, rather than hand-maintained and
  scoped to base only.

### Documentation

- `README.md`, `AGENTS.md` and `CONTRIBUTING.md` cover all 22 dialects and the current architecture.
  `README.md` also gained a plain, factual fork-history and attribution note explaining its lineage
  from `tree-sitter-sql`/`tree-sitter-sequel` through `tree-sitter-sql-extended`.
- `docs/usage.md` is a new page covering install/import for every registry this project publishes to
  (npm, crates.io, PyPI), later extended to document the per-dialect Cargo features and Node/Python
  lazy loading, and finally to cover all six binding targets: Rust/Node/Python/Go/Swift/CMake+C.

## [0.3.11](https://github.com/relativelyunknown/tree-sitter-sql-polyglot/releases/tag/v0.3.11) (2026-05-23)
