...
8. Wire the dialect into the compiled Rust/Node/Python/Go/Swift/CMake packages (this is separate from
   step 4's per-dialect `<dialect>/tree-sitter.json`, which only registers it for standalone
   tree-sitter CLI use):
   ...
   - Add `my_dialect` to the `DIALECT_DIRS` list at the top of `scripts/generate-bindings.js`, then run
     ...
     `bindings/go/my_dialect/{binding.go,binding_test.go}`, `Package.swift` +
     `bindings/swift/TreeSitterSqlMyDialect{,Tests}/*`,
     `bindings/c/tree_sitter/tree-sitter-sql-my_dialect.h` + `bindings/c/tree-sitter-sql-my_dialect.pc.in`,
     and a new `option(TREE_SITTER_SQL_MY_DIALECT ...)` block appended to the root `CMakeLists.txt`
     (also fully regenerated by this script - see the "C/CMake" section in
     `scripts/generate-bindings.js`). The new dialect gets its own Cargo
