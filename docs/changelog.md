...
- **Base (ANSI):** typed temporal literals (`DATE '…'`, `TIME '…'`, `TIMESTAMP '…'`), datetime value functions (`CURRENT_DATE`, `CURRENT_TIME`, `CURRENT_TIMESTAMP`, `LOCALTIME`, `LOCALTIMESTAMP`), and `LIKE … ESCAPE '…'`
- **Tooling:** replaced the `scorecard.py`/`corroborate.py`/`parse_rate.py`/`diff_parse.py` scripts with one corroborated coverage tool (`tools/coverage.py`) that cross-checks every feature probe against independent reference parsers (SQLGlot, ANTLR grammars-v4, pglast, sqlfluff) and scores the base grammar against ISO Core — surfacing "suspect" probes (ours accepts, no reference does) and confirmed gaps (e.g. cursors)
